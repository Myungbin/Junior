...
# 검정통계량 T0 계산
cal_t_stat <- function(x1, x2, diff_mu) {
...
cal_t_stat(gain_treat, gain_control, diff_mu = 0)
...
library(readxl)
setwd("C:/Users/kogeu/Desktop/[2021-2학기] 통계적 사고와 이해 실습/13주차 실습/")
...
unique(lifelog$금주여부)
lifelog[lifelog$금주여부 == FALSE, ]

bfp_last <- c()

for (i in 1:length(unq_user_id)) {
  bfp_by_user <- head(lifelog[lifelog$user_id == unq_user_id[i], c('체지방률', '금주여부')], n = 1)
...
unique(bfp_last$금주여부)
prop.table(table(bfp_last$금주여부))
bfp_last$금주여부 <- factor(bfp_last$금주여부, levels = c(TRUE, FALSE), labels = c("금주","음주"))



# 분포 살펴보기
hist(bfp_last$체지방률, freq = F, main = '전체의 체지방률의 분포')
lines(density(bfp_last$체지방률), col = 'red')

shapiro.test(bfp_last$체지방률)  

summary(bfp_last$체지방률)
x_ax = seq(10, 55, by = 5)

hist1 = hist(bfp_last[bfp_last$금주여부 == "금주", "체지방률"], breaks=x_ax, plot = FALSE)
hist2 = hist(bfp_last[bfp_last$금주여부 == "음주", "체지방률"], breaks=x_ax, plot = FALSE)

plot(hist1, col=adjustcolor("red", alpha=0.5),  freq = F, ann = F)
plot(hist2, col=adjustcolor("blue", alpha=0.5), freq = F, add = TRUE)
lines(density(bfp_last[bfp_last$금주여부 == "금주", "체지방률"]), col = 'red')
lines(density(bfp_last[bfp_last$금주여부 == "음주", "체지방률"]), col = 'blue')

title(main="금주여부에 따른 체지방률의 분포")
legend("topright",legend=c("금주","음주"), fill=c("red","blue"))

shapiro.test(bfp_last[bfp_last$금주여부 == "금주", "체지방률"])
shapiro.test(bfp_last[bfp_last$금주여부 == "음주", "체지방률"])


# Two sample t-test
par(mfrow = c(1,2))
boxplot(bfp_last[bfp_last$금주여부 == "금주", "체지방률"], ylim = c(0, 55), main = '금주일 때의 체질방률의 boxplot')
points(mean(bfp_last[bfp_last$금주여부 == "금주", "체지방률"]), pch = 18, col = 'red')
boxplot(bfp_last[bfp_last$금주여부 == "음주", "체지방률"], ylim = c(0, 55), main = '음주일 때의 체질방률의 boxplot')
points(mean(bfp_last[bfp_last$금주여부 == "음주", "체지방률"]), pch = 18, col = 'red')

var.test(체지방률 ~ 금주여부, data = bfp_last, conf.level = 0.95)
t.test(체지방률 ~ 금주여부, data = bfp_last, 
       mu = 0, conf.level = 0.95, alternative = "two.sided", var.equal = TRUE)
